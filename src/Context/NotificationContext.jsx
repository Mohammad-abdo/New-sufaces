...
export default function NotificationProvider(props){
  ...
  try {
    ...
    queryClient.invalidateQueries(['AllNotification']); // عشان يجيب التحديث من السيرفر
  } catch (error) {
  ...
  .catch(error => {
    ...
    return <NotificationtContext.Provider value={{notifications: data || [] , markAllRead , switchLanguage, hideNotificationCount}}>
        {props.children}
