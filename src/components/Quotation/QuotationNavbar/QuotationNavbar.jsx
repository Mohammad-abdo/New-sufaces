...
export default function QuotationNavbar() {
  ...
  async function reedAllMessage() {
    if (notifications?.length > 0) {
      await markAllRead(); // يخلي كل النوتيفيكشن "read" + يحدّث الكاش
      hideNotificationCount(); // يخفي العداد فوراً
    }
    setOpen((prev) => !prev); // يفتح أو يقفل الـ dropdown
  }
  ...
  // 🟢 notifications unread count (لو محتاجه)
